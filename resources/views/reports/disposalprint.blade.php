...
<div class="br-mainpanel"> 
 <div class="br-pagebody tx-roboto tx-inverse">
    <div class="br-section-wrapper">
      ...
      <h6 class="tx-bold tx-center">PROPERTY DISPOSAL REPORT</h6>
    </div>
       </div>
       <div class="row pd-t-5">
         <div class="col-sm-6">Entity Name: {{$disposal->entity}}</div>          
         <div class="col-sm-6">PDR No: {{$disposal->ref_no}}</div> 
         <div class="col-sm-6">Fund Cluster: {{$disposal->fund_cluster}}</div> 
         <div class="col-sm-6">Date: {{$disposal->date}}</div> 
       </div>
     <div class="row bd bd-gray-900 bd-2">
      <div class="wd-100p pd-b-20">
         <div class="col-sm-12 pd-b-20">Model of Disposal: </div>
         <div class="row pd-l-35">
          @foreach($disposal_modes as $tr)
            @if($tr->id == $disposal->disposal_mode)
        ...
        <div class="col-sm-7 pd-t-5"></div>
          @if($disposal->disposal_others != null)
            <div class="col-sm-5 bd-b">{{$disposal->transfer_others}}</div>
          @endif
        </div>
      </div>
    </div>
        ...
        <div class="wd-100p">
         <table class="table-bordered tx-12 wd-100p">
          ...
          <tbody class="bd">
            @foreach($items as $it)
              <tr>
                <td class="wd-10p">{{$it['date']}}</td>
                <td class="wd-10p">{{$it['no']}}</td>
                <td class="wd-30p">{{$it['name']}}</td>
                <td class="wd-5p">{{$it['qty']}}</td>
                <td class="wd-10p">{{$it['unit_cost']}}</td>
              </tr>
            @endforeach

            <tr>
              <td colspan="3" class="tx-right tx-bold"></td>
              <td>TOTAL AMOUNT: </td>
              <td  class="tx-right tx-bold">{{number_format($disposal->total,2)}}</td>
            </tr>
            <tr>
              <td colspan="3" class="tx-right tx-bold"></td>
              <td>If mode of disposal is Sale: </td>
              <td  class="tx-right tx-bold"></td>
            </tr>
            <tr>
              <td colspan="3" class="tx-right tx-bold"></td>
              <td>Date of Sale: </td>
              <td  class="tx-right tx-bold">{{$disposal->date_sale}}</td>
            </tr>
            <tr>
              <td colspan="3" class="tx-right tx-bold"></td>
              <td>Amount: </td>
            ...
            <tr>
              <td colspan="3" class="tx-right tx-bold"></td>
              <td>OR: </td>
              <td  class="tx-right tx-bold">{{$disposal->or_no}}</td>
            </tr>
          </tbody>
         </table>
            ...
            <div class="col-sm-3 tx-12">
              <div class="row">
                ...
                <div class="col-sm-12 bd-b">{{$disposal->approved_by}}</div>
                <div class="col-sm-12 bd-b">{{$disposal->appos}}</div>
                <div class="col-sm-12 bd-b">{{$disposal->date_approved}}</div>
              </div>
            </div>
            ...
            <div class="col-sm-3 tx-12">
              <div class="row">
                ...
                <div class="col-sm-12 bd-b">{{$disposal->issuedby}}</div>
                <div class="col-sm-12 bd-b">{{$disposal->ispos}}</div>
                <div class="col-sm-12 bd-b">{{$disposal->date_issued}}</div>
              </div>
            </div>
