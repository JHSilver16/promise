...
<div class="br-mainpanel"> 
 <div class="br-pagebody tx-roboto tx-inverse">
    <div class="br-section-wrapper">
      ...
      <h6 class="tx-bold tx-center">INVENTORY OF SUPPLIES AND MATERIALS</h6>
    </div>
       </div>
       <div class="row pd-t-5">
         <div class="col-sm pd-r-5">Entity Name: {{$info[1]->name}}</div>          
         
         <div class="col-sm-5"></div>
         <div class="col-sm">Serial No: </div> 
       </div>
       <div class="row pd-t-5">
         <div class="col-sm pd-r-5">Fund Cluster: 101</div>          
         
         <div class="col-sm-5"></div>
         <div class="col-sm">Date:{{$info[0]}} </div> 
       </div>
         ...
         <table class="table-bordered tx-12 wd-100p">
          ...
          <tbody class="bd">
            ...
            <tr>
              ...
              <td>{{$ar['qty']}}</td>
              <td>{{$ar['unit_cost']}}</td>
              <td class="amount">{{$ar['amount']}}</td>
            </tr>
            @endforeach
          </tbody>
          <tfoot>
            <tr>
              <td colspan="6" class="tx-right" >Total </td>
              <td id="amount"></td>
            </tr>
          </tfoot>
         </table>
          ...
          <div class="row tx-12 pd-t-5 pd-b-5 bd bd-gray-900 bd-2">
            <div class="col-sm-6 bd-r bd-gray-900 bd-2">
                
                <div class="pd-b-20">I hereby certify to the corrections of the above information: </div>
              <div class="row tx-center">
              
                <div class="col-sm-12 tx-bold">{{$ao->first_name.' '.$ao->middle_name[0].'. '.$ao->last_name}}</div>
                <div class="col-sm-12 ">{{$ao->position}} </div>
              </div>
            </div>
            <div class="col-sm-6 tx-12">
                
              <div class="pd-b-20">Noted by: </div>
              <div class="row tx-center">

                <div class="col-sm-9 tx-bold">{{$cao->first_name.' '.$cao->middle_name[0].'. '.$cao->last_name}}</div>
                <div class="col-sm-3 ">_________</div>
                <div class="col-sm-9 ">{{$cao->position}}</div>
...
<script type="text/javascript">
  ...
  var amountsum = document.getElementsByClassName("amount");
  var sum = 0;
  for (i=0; i<amountsum.length; i++){
    sum = sum + parseFloat(amountsum[i].innerHTML)
  }
