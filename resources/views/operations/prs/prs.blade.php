            ...
            <div class="row">
              ...
              <div class="col-sm-5">
                 <select class="form-control form-layout-2" id="status">
                  ...
                  <option value="PAID WITH PETTY CASH">Petty Cash</option>
                  <option value="PAID WITH CASH ADVANCE">Cash Advance</option>
                  <option value="PAID WITH REIMBURSEMENTS">Reimbursements</option>
              ...
              <table class="table table-bordered" id="products">
                  <thead class="thead-colored thead-dark tx-black">
                   ...
                   <tbody class="tx-inverse">
                     @foreach($prs as $pr)
                       <tr>
                       <td><input type="checkbox" value="{{$pr['id']}}" class="chk"></td>
                        <td>{{$pr['no']}}</td>
                        <td>{{$pr['employee']}}</td>
                        <td>{{$pr['purpose']}}</td>
                        <td>{{$pr['items']}}</td>
                        ...
                        <td><div class="btn-group" role="group" aria-label="Basic example">
                          @if($pr['status'] == 'PENDING')
                           <button type="button" class="btn btn-success edit" data-id="{{$pr['id']}}"><i class="fa fa-edit"></i> Edit PR</button>
                           <div class="dropdown">
                              <button class="btn btn-warning dropdown-toggle" type="button" data-toggle="dropdown">Charge with
                              <span class="caret"></span></button>
                             <div class="dropdown-menu pd-10 wd-300">
                                <nav class="nav nav-style-2 flex-column">
                                  <a href="{{route('updatepr', ['id' => $pr['id'], 'status' => 'Petty Cash', 'type'=>'charge',  'remarks'=>'none'])}}" class="nav-link"><i class="fa fa-money-check fa-xs"></i> Petty Cash</a>
                                  <a href="{{route('updatepr', ['id' => $pr['id'], 'status' => 'Cash Advance', 'type'=>'charge',  'remarks'=>'none'])}}" class="nav-link"><i class="fa fa-clipboard-list fa-xs"></i>Cash Advance</a>
                                  <a href="{{route('updatepr', ['id' => $pr['id'], 'status' => 'Reimbursements', 'type'=>'charge',  'remarks'=>'none'])}}" class="nav-link"><i class="fa fa-th-list fa-xs"></i> Reimbursements</a>
                           ...
                           <div class="dropdown">
                              <button class="btn btn-primary dropdown-toggle" type="button" data-toggle="dropdown"><i class="fa fa-plus-square fa-xs"></i> Create Form
                              <span class="caret"></span></button>
                             <div class="dropdown-menu pd-10 wd-300 tx-14">
                                <nav class="nav nav-style-2 flex-column">
                                   @if(($pr['status'] == 'PENDING') || ($pr['status'] == 'With RFQ'))
                                  <a class="nav-link add" data-type="rfq"  title="Edit" data-date="{{$pr['date']}}" data-id="{{$pr['id']}}" data-purpose="{{$pr['purpose']}}" data-name="{{$pr['no']}}"><i class="fa fa-plus-square fa-xs"></i> Request for Quotation</a>
                                  @endif
                                   @if($pr['status'] == 'PENDING')
                                  <a class="nav-link addpo" data-type="po" data-id="{{$pr['id']}}" data-purpose="{{$pr['purpose']}}" data-name="{{$pr['no']}}"><i class="fa fa-plus-square fa-xs"></i> Create PO</a>
                                     @endif                               
                                </nav>
                              </div><!-- dropdown-menu -->
                            </div>
         ...
         <div class="col-md-12 mg-t--1 mg-md-t-0 pno">
          ...
          <div class="col-md-12 mg-t--1 mg-md-t-0 pno">
            <div class="form-group mg-md-l--1">
              ...
              <select class="form-control select2-show-search " name="procmode" style="width: 100%" data-placeholder="Choose one (with searchbox)" tabindex="-1" aria-hidden="true">
...
var prid = [];
 $(document).on('click', '.update', function() {
       ...
       if($('.chk:checked').length == 0){
        swal("Warning!", "Please check at least one PR.", "warning");
       }
       else{
        $(".chk:checkbox:checked").each(function() {
          prid.push($(this).val())
        })
       $.post("{{route('updateprlist')}}",
        {
        ...
        function(data,status){
            if(data == 'ok'){
              swal("Success!", "PRs have been updated", "success");
