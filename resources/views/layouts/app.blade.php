...
setInterval(function(){ 
   ...
   .then((responseText)=>{
    ...
    var hts = '';
    $('#prslist').empty();
    const prs = JSON.parse(responseText)[0];
    const pos = JSON.parse(responseText)[1];

    var audio = new Audio("{{asset('audio/notif.mp3')}}");
    var prevcount = document.getElementById('notif').innerHTML;
    var prevpr = document.getElementById('prcount').innerHTML;
    document.getElementById('prcount').innerHTML = prs.length;
    console.log(prevcount < (prs.length+pos.length))
  

    if(prevpr < prs.length){
      showNotification((prs.length - prevpr)+' PENDING PR. Please check the notification panel.')
    }

    document.getElementById('notif').innerHTML = prs.length+pos.length
        prs.forEach(function(pr){
          ht += '<a href="{{route("prs")}}" class="contact-list-link new">'+
              '<div class="d-flex">'+
                '<div class="contact-person">'+
                  '<p class="mg-b-0">'+pr['ref_no']+'</p>'+
                 ' <span class="tx-12 op-5 tx-white d-inline-block">'+pr['first_name']+' '+pr['last_name']+'</span><br>'+
                  ' <span class="tx-12 op-5 tx-white d-inline-block">'+pr['purpose'].substring(0,15)+'</span>'+
               ' </div>'+
                '<span class="badge badge-light">'+pr['date']+'</span>'+
              '</div><!-- d-flex -->'+
            '</a><!-- contact-list-link -->';

        })
        pos.forEach(function(pr){
          hts += '<a href="{{route("pos")}}" class="contact-list-link new">'+
              '<div class="d-flex">'+
                '<div class="contact-person">'+
                  '<p class="mg-b-0">'+pr['ref_no']+'</p>'+
                 ' <span class="tx-12 op-5 tx-white d-inline-block">PR No:'+pr['prno']+'</span><br>'+
                 ' <span class="tx-12 op-5 tx-white d-inline-block">PR No:'+pr['purpose']+'</span><br>'+
               ' </div>'+
                '<span class="badge badge-light">'+pr['date']+'</span>'+
              '</div><!-- d-flex -->'+
            '</a><!-- contact-list-link -->';
...
function showNotification(msg) {
   const notification = new Notification("New PENDING incoming", {
      body: msg,
